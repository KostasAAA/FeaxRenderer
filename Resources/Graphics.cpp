...
void Graphics::ResourceBarrierBegin(GraphicsContext& context)
{
	context.m_barriers.clear();
}


void Graphics::ResourceBarrierEnd(GraphicsContext& context)
{
	size_t noofBarriers = context.m_barriers.size();

	if (noofBarriers > 0)
	{
		context.m_commandList->ResourceBarrier(noofBarriers, context.m_barriers.data());
	}
}
